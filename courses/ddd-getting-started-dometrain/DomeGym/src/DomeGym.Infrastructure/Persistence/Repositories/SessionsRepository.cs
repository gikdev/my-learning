...
file static class DbContextSessionExtensions {
    public static IQueryable<Session> WhereBetweenDateAndTimes(this IQueryable<Session> query, DateTime? start,
        DateTime? end) {
        if (start == null && end == null) return query;
    ...
    public static IQueryable<Session>
        WhereOfCategory(this IQueryable<Session> query, List<SessionCategory>? categories) {
        if (categories is null || categories.Count == 0) return query;

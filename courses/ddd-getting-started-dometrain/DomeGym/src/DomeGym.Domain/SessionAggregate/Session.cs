...
public class Session : AggregateRoot {
    ...
    public ErrorOr<Success> CancelReservation(Participant participant, IDateTimeProvider dateTimeProvider) {
        ...
        if (reservation is null) return Error.NotFound(description: "Reservation not found");

        if (IsPastSession(dateTimeProvider.UtcNow)) return SessionErrors.CannotCancelPastSession;

        if (IsTooCloseToSession(dateTimeProvider.UtcNow)) return SessionErrors.CannotCancelReservationTooCloseToSession;
    ...
    public ErrorOr<Success> ReserveSpot(Participant participant) {
        if (_reservations.Count >= MaxParticipants) return SessionErrors.CannotHaveMoreReservationsThanParticipants;

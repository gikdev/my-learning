...
public class Room : AggregateRoot {
    ...
    public ErrorOr<Success> ScheduleSession(Session session) {
        ...
        if (!_sessionIdsByDate.ContainsKey(session.Date)) _sessionIdsByDate[session.Date] = new List<Guid>();

        var dailySessions = _sessionIdsByDate[session.Date];

        if (dailySessions.Count >= _maxDailySessions) return RoomErrors.CannotHaveMoreSessionThanSubscriptionAllows;

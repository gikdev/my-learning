...
public class Gym : AggregateRoot {
    ...
    public ErrorOr<Success> AddRoom(Room room) {
        if (_roomIds.Contains(room.Id)) return Error.Conflict(description: "Room already exists in gym");
    ...
    public ErrorOr<Success> AddTrainer(Trainer trainer) {
        if (_trainerIds.Contains(trainer.Id)) return Error.Conflict(description: "Trainer already assigned to gym");
    ...
    public ErrorOr<Success> RemoveRoom(Room room) {
        if (!_roomIds.Contains(room.Id)) return Error.NotFound(description: "Room not found");

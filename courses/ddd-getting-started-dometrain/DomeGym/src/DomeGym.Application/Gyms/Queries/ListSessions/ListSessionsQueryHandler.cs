...
public class ListSessionsQueryHandler : IRequestHandler<ListSessionsQuery, ErrorOr<List<Session>>> {
    ...
    public ListSessionsQueryHandler(IGymsRepository gymsRepository, ISubscriptionsRepository subscriptionsRepository,
    ...
    public async Task<ErrorOr<List<Session>>> Handle(ListSessionsQuery query, CancellationToken cancellationToken) {
        ...
        if (!subscription.HasGym(query.GymId)) return Error.NotFound(description: "Gym not found");

        if (!await _gymsRepository.ExistsAsync(query.GymId)) return Error.NotFound(description: "Gym not found");

        return await _sessionsRepository.ListByGymIdAsync(query.GymId, query.StartDateTime, query.EndDateTime,
            query.Categories);
    }
}

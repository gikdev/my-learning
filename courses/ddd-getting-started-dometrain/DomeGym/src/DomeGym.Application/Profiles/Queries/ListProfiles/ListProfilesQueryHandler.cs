...
public class ListProfilesQueryHandler : IRequestHandler<ListProfilesQuery, ErrorOr<List<Profile>>> {
    ...
    public ListProfilesQueryHandler(ITrainersRepository trainersRepository,
        IParticipantsRepository participantsRepository, IAdminsRepository adminsRepository) {
    ...
    public async Task<ErrorOr<List<Profile>>> Handle(ListProfilesQuery request, CancellationToken cancellationToken) {
        ...
        if (profiles.Count == 0) return Error.NotFound(description: "User not found");

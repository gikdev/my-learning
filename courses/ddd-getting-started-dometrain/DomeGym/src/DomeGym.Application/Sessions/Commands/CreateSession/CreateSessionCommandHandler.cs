...
public class CreateSessionCommandHandler : IRequestHandler<CreateSessionCommand, ErrorOr<Session>> {
    ...
    public async Task<ErrorOr<Session>> Handle(CreateSessionCommand command, CancellationToken cancellationToken) {
        ...
        if (room is null) return Error.NotFound(description: "Room not found");

        var trainer = await _trainersRepository.GetByIdAsync(command.TrainerId);

        if (trainer is null) return Error.NotFound(description: "Trainer not found");
        ...
        var session = new Session(
            ...
            command.RoomId,
            command.TrainerId,
            DateOnly.FromDateTime(command.StartDateTime),
            createTimeRangeResult.Value,
            command.Categories);

        var scheduleSessionResult = room.ScheduleSession(session);

        if (scheduleSessionResult.IsError) return scheduleSessionResult.Errors;

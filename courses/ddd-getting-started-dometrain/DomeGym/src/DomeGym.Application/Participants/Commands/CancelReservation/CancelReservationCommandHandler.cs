...
public class CancelReservationCommandHandler : IRequestHandler<CancelReservationCommand, ErrorOr<Deleted>> {
    ...
    public CancelReservationCommandHandler(IParticipantsRepository participantsRepository,
        ISessionsRepository sessionsRepository, IDateTimeProvider dateTimeProvider) {
    ...
    public async Task<ErrorOr<Deleted>> Handle(CancelReservationCommand command, CancellationToken cancellationToken) {
        ...
        if (session is null) return Error.NotFound(description: "Session not found");
        ...
        if (participant is null) return Error.NotFound(description: "Participant not found");
        ...
        if (cancelReservationResult.IsError) return cancelReservationResult.Errors;

...
using Moduben.Common.Application.Authorization;
...
namespace Moduben.Common.Infrastructure.Authorization;
...
internal sealed class CustomClaimsTransformation(IServiceScopeFactory serviceScopeFactory) : IClaimsTransformation {
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal) {
        ...
        if (result.IsFailure) {
            throw new ModubenException(nameof(IPermissionService.GetUserPermissionsAsync), result.Error);
        }
